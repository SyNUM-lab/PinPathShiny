...
.mapColors <- function(nodes_df,
  ...
  # Get gene/protein IDs of all nodes
  geneIDs_nodes <- nodes_df[,c("Database", "ID", "GraphId1")]
  
  # Filter geneIDs
  acceptedDB <- c("Ensembl", "Entrez Gene", "Uniprot-TrEMBL", "HGNC")
  geneIDs_fil <- geneIDs_nodes[geneIDs_nodes$Database %in% acceptedDB,]
  ...
  for (db in unique(inputDB)){
    ...
    for (i in 1:length(databases)){
      ...
      if (keytype == db){
        ...
        temp <- tryCatch({
          temp <- geneIDs_fil[geneIDs_fil$Database == keytype, ]
          ann <- suppressMessages(AnnotationDbi::select(BiocGenerics::get(annPkg), 
    ...
    nodeAnn_all <- rbind.data.frame(nodeAnn_all, nodeAnn)
  }
