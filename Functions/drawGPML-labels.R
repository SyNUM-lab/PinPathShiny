...
.prepareLabels <- function(dataLabels){
    ...
    data.frame(
      ...
      nLine = ifelse(
        sum(stringr::str_detect(
          unlist(dataLabels[which(names(dataLabels) == "Attribute")]), 
          "Double")) == 0, 
        "Single", "Double"),
  ...
  labels_df$FontFace[
    (!is.na(labels_df$FontWeight) & 
       is.na(labels_df$FontStyle))] <- labels_df$FontWeight[
         (!is.na(labels_df$FontWeight) & is.na(labels_df$FontStyle))]
  labels_df$FontFace[
    (is.na(labels_df$FontWeight) & 
       !is.na(labels_df$FontStyle))] <- labels_df$FontStyle[
         (is.na(labels_df$FontWeight) & !is.na(labels_df$FontStyle))]
  labels_df$FontFace[
