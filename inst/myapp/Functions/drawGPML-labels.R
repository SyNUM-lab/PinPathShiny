...
labelFUN <- function(dataLabels){
    data.frame(
        ...
        FillColor = ifelse(
            is.na(as.character(dataLabels$Graphics["FillColor"])),"white",
            paste0("#",as.character(dataLabels$Graphics["FillColor"]))),
        Alpha = 1,
        Color = ifelse(
            is.na(as.character(dataLabels$Graphics["Color"])),"black",
        ...
        Valign = ifelse(
            as.character(dataLabels$Graphics["Valign"]) == "Middle",0.5,
            ifelse(as.character(dataLabels$Graphics["Valign"])=="Top",1,0)),
        Align = ifelse(
            as.character(dataLabels$Graphics["Align"]) == "Middle",0.5,
        ...
        LineThickness = as.numeric(ifelse(
            is.na(dataLabels$Graphics["LineThickness"]),1,
            dataLabels$Graphics["LineThickness"]))/2,
        LineStyle = as.character(ifelse(is.na(
            dataLabels$Graphics["LineStyle"]), "solid",
